...
for (file in all_files) {
  ...
  for (i in 1:100) {
    ...
    shuffledCountsT<-apply(ResampledcountsT, 1, sample)
  ...
  save_dir<-"NBResampleShuffleInTaxonDump/RDP/"
...
for (file in all_files) {
  ...
  save_dir<-"NBResampleShuffleInTaxonDump/dada2/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  save_dir<-"NBResampleShuffleInTaxonDump/WGS/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  for (i in 1:100) {
    ...
    meta<-meta[intersect(colnames(ResampledcountsT), rownames(meta)), , drop = F]
  ...
  save_dir<-"NBResampleShuffleInTaxonDump/RNAseq/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
