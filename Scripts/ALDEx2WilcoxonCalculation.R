...
for (file in all_files) {
  ...
  results<-calcALDEx2Wilcoxon(countsT, meta)

  save_dir<-"PkgResults/RDP/ALDEx2Wilcoxon/"
  if (file.exists(file.path(save_dir))){
    write.table(results, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_ALDEx2W.txt"), sep = "\t", row.names = T)
  } else {
...
for (file in all_files) {
  ...
  save_dir<-"PkgResults/DADA2/ALDEx2Wilcoxon/"
  if (file.exists(file.path(save_dir))){
    write.table(results, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_ALDEx2W.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  save_dir<-"PkgResults/WGS/ALDEx2Wilcoxon/"
  if (file.exists(file.path(save_dir))){
    write.table(results, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_ALDEx2W.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  save_dir<-"PkgResults/RNAseq/ALDEx2Wilcoxon/"
  if (file.exists(file.path(save_dir))){
    write.table(results, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_ALDEx2W.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
