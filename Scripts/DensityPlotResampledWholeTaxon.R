# density plot of resample whole taxon
library(ggplot2)
library(ggExtra)
library(patchwork)
...
pdf("Plots/DensityWholeTaxon(Assal).pdf", width=14, height=7)
for (i in 1:nrow(countsT)) {
  ...
  # resampled
  data <- data.frame(value = unlist(newT[taxon, ]))

  p2<-ggplot(data, aes(x = value)) +
    geom_density(fill = "green", alpha = 0.5) +  # Add density plot with transparency
    labs(title = "Density Distribution of Whole taxon",
         x = "Value",
         y = "Density") +
    theme_classic()
