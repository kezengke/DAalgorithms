...
for (file in all_files) {
  ...
  png(paste0("Plots/RDP/Rnormx1BeforeAfterLog10PvPPlots(", name, ").png"), width=3400, height=2400, res = 300)
...
for (file in all_files) {
  countsT<-LoadCountsT(file)
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/dada2/Rnormx1BeforeAfterLog10PvPPlots(", name, ").png"), width=3400, height=2400, res = 300)
  meta<-LoadMeta(paste0("MetaData/metadata_", name, ".txt"))
...
for (file in all_files) {
  countsT<-LoadCountsT(file)
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/WGS/Rnormx1BeforeAfterLog10PvPPlots(", name, ").png"), width=3400, height=2400, res = 300)
  meta<-LoadMeta(paste0("MetaData/metadata_", name, ".txt"))
