# resample whole taxon then shuffle counts in every sample 100x and test all pval
rm(list = ls())
set.seed(9527)
...
for (file in all_files) {
  ...
  save_dir<-"ResampleWholeTaxonShuffleInSampleDump/RDP/"
...
for (file in all_files) {
  ...
  save_dir<-"ResampleWholeTaxonShuffleInSampleDump/dada2/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  save_dir<-"ResampleWholeTaxonShuffleInSampleDump/WGS/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  save_dir<-"ResampleWholeTaxonShuffleInSampleDump/RNAseq/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
