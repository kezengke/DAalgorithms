#shuffle counts in every taxon 100x and test all pval for resampled whole taxon of counts tables
rm(list = ls())
set.seed(9527)
...
for (file in all_files) {
  ...
  save_dir<-"ResampleWholeTaxonShuffleInTaxonDump/RDP/"
...
for (file in all_files) {
  ...
  save_dir<-"ResampleWholeTaxonShuffleInTaxonDump/dada2/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  save_dir<-"ResampleWholeTaxonShuffleInTaxonDump/WGS/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  save_dir<-"ResampleWholeTaxonShuffleInTaxonDump/RNAseq/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
