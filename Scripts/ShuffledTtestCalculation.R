...
for (file in all_files) {
  ...
  results<-calcTtest(countsTnorm, meta)


  main_dir<-"PkgResults/RDP"
  sub_dir<-"shuffledttest/"
  if (file.exists(file.path(main_dir, sub_dir))){
    write.table(results, paste0(file.path(main_dir, sub_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
