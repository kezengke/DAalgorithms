#testing normality before shuffling in counts table
rm(list = ls())
set.seed(9527)
...
for (file in all_files) {
  ...
  png(paste0("Plots/RDP/ResampledShuffledCountsTKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 500)
  ...
  for (i in 1:100) {
    ...
    p1<-PvalHistogram(pvals1, "#f6ad22", title = "Resampled Shuffled Counts Table KS Test \n(condition 1)")
    p2<-PvalHistogram(pvals2, "#f6ad22", title = "Resampled Shuffled Counts Table KS Test \n(condition 2)")
...
for (file in all_files) {
  combined_plots<-NULL
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/DADA2/ResampledShuffledCountsTKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 500)
...
for (file in all_files) {
  combined_plots<-NULL
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/WGS/ResampledShuffledCountsTKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 500)
...
for (file in all_files) {
  combined_plots<-NULL
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/RNAseq/ResampledShuffledCountsTKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 500)
