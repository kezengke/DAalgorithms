# resample whole taxon then shuffle counts in every sample 100x and test all pval (negative binomial distribution)
rm(list = ls())
set.seed(9527)
...
for (file in all_files) {
  ...
  save_dir<-"NBResampleWholeTaxonShuffleInSampleDump/RDP/"
...
for (file in all_files) {
  ...
  save_dir<-"NBResampleWholeTaxonShuffleInSampleDump/dada2/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  save_dir<-"NBResampleWholeTaxonShuffleInSampleDump/WGS/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
...
for (file in all_files) {
  ...
  save_dir<-"NBResampleWholeTaxonShuffleInSampleDump/RNAseq/"
  if (file.exists(file.path(save_dir))){
    write.table(ttestpvals, paste0(file.path(save_dir), gsub(basename(file), pattern=".txt$", replacement=""), "_t.txt"), sep = "\t", row.names = T)
  } else {
    dir.create(file.path(save_dir))
