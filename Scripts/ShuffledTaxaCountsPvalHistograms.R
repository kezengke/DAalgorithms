...
for (file in all_files) {
  ...
  png(paste0("Plots/RDP/(", name, ")ShuffledTaxaMethodsPvalueHistograms.png"), width= 4800, height=1200, res = 300)
  ...
  newRawT<-apply(RawcountsT, 2, sample)
  rownames(newRawT)<-rownames(RawcountsT)
  colnames(newRawT)<-colnames(RawcountsT)
  newRawT<-data.frame(newRawT)
...
for (file in all_files) {
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/dada2/(", name, ")ShuffledTaxaMethodsPvalueHistograms.png"), width= 4800, height=1200, res = 300)
...
for (file in all_files) {
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/WGS/(", name, ")ShuffledTaxaMethodsPvalueHistograms.png"), width= 4800, height=1200, res = 300)
  ...
  rownames(newRawT)<-rownames(RawcountsT)
  colnames(newRawT)<-colnames(RawcountsT)
  newRawT<-data.frame(newRawT, check.names = F)
