#testing normality before shuffling in sample tags
rm(list = ls())
set.seed(9527)
...
for (file in all_files) {
  ...
  png(paste0("Plots/RDP/ResampledShuffledSampleTagsKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 300)
  ...
  for (i in 1:100) {
    ...
    CountsT1<-RawcountsT[, shuffleMeta$conditions == group1, drop = F]
    CountsT2<-RawcountsT[, shuffleMeta$conditions == group2, drop = F]

    CountsT1<-CountsT1[apply(CountsT1, 1, function(row) any(row != 0)), ]
    CountsT2<-CountsT2[apply(CountsT2, 1, function(row) any(row != 0)), ]

    pvals1<-data.frame(apply(CountsT1, 1, NormalityKS))
    pvals2<-data.frame(apply(CountsT2, 1, NormalityKS))

    colnames(pvals1)<-"pval"
    colnames(pvals2)<-"pval"

    p1<-PvalHistogram(pvals1, "#73add3", title = "Resampled Shuffled Sample Tags KS Test \n(condition 1)")
    p2<-PvalHistogram(pvals2, "#73add3", title = "Resampled Shuffled Sample Tags KS Test \n(condition 2)")
...
for (file in all_files) {
  combined_plots<-NULL
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/DADA2/ResampledShuffledSampleTagsKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 300)
...
for (file in all_files) {
  combined_plots<-NULL
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/WGS/ResampledShuffledSampleTagsKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 300)
...
for (file in all_files) {
  combined_plots<-NULL
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/RNAseq/ResampledShuffledSampleTagsKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 300)
