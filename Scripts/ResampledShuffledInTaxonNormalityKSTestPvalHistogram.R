#testing normality before shuffling
rm(list = ls())
set.seed(9527)
...
for (file in all_files) {
  ...
  png(paste0("Plots/RDP/ResampledShuffledInTaxonKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 300)
  ...
  for (i in 1:100) {
    ...
    p1<-PvalHistogram(pvals1, "#8FB257", title = "Resampled Shuffled In Taxon KS Test \n(condition 1)")
    p2<-PvalHistogram(pvals2, "#8FB257", title = "Resampled Shuffled In Taxon KS Test \n(condition 2)")
...
for (file in all_files) {
  combined_plots<-NULL
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/DADA2/ResampledShuffledInTaxonKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 300)
...
for (file in all_files) {
  combined_plots<-NULL
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/WGS/ResampledShuffledInTaxonKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 300)
...
for (file in all_files) {
  combined_plots<-NULL
  name <- gsub(basename(file), pattern=".txt$", replacement="")
  png(paste0("Plots/RNAseq/ResampledShuffledInTaxonKSNormalityTest(", name,").png"), width=1200*20, height=1200*10, res = 300)
